...
module.exports.getDirectorySubDirectoriesAndFiles = async function (directory) {
  try {
    ...
    if (dirs && dirs.length > 0) {
      data.directories = await Promise.all(
        dirs.map(async (dir) => {
...
module.exports.getDirectoryContent = async function (id) {
  try {
    ...
    const data = await module.exports.getDirectorySubDirectoriesAndFiles(
      directory
    );
...
module.exports.ifDirectoryExist = async function (project, parentDir, name){
